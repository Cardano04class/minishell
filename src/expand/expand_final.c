...
/*   Updated: 2024/12/12 18:33:59 by mamir            ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
{
    // First pass: calculate exact length needed for expanded content
    ...
    // Allocate exact space for expanded content
    char *expanded_content = _malloc(expanded_len + 1, 'm');
    if (!expanded_content)
        return NULL;

    // Second pass: actual expansion
    i = 0;
    int j = 0;
    in_single_quote = 0;
    in_double_quote = 0;
...
void expand(t_env *env, t_list **list)
{
    ...
    // printf("before expand_variables_in_list :****\n");
    // debug_list(*list);
    expand_variables_in_list(env, list);
    // printf("after expand_variables_in_list****\n");
    // debug_list(*list);
